...
class News_manage extends Fuel_base_controller {
	...
	function lists($dataStart=0)
	{
		...
		$vars['create_url'] = $base_url.'fuel/news/create?type='.$type.'&cate='.$cate;
	...
	function create()
	{
		// $is_sup = $this->input->get_post('sup'); 
		$type = $this->input->get_post('type'); 
		// if (!$is_sup) {
	...
	function do_create()
	{
		...
		$root_img_path  = assets_server_path('news_img/'.$post_arr['type']."/".$post_arr['lang']."/");
		$root_file_path = assets_server_path('news_file/'.$post_arr['type']."/".$post_arr['lang']."/");
		if (!file_exists($root_img_path)) {
		    mkdir($root_img_path, 0777, true);
		}
		if (!file_exists($root_file_path)) {
		    mkdir($root_file_path, 0777, true);
		}
		 
		$post_arr["content"] = htmlspecialchars($post_arr["content"]);	

		$module_uri = base_url().$this->module_uri.'?type='.$post_arr['type'];
		 
		//image
		$config['upload_path'] = $root_img_path;
		...
		//file
		$config['upload_path'] = $root_file_path;
		...
		{
			$data = array('upload_data'=>$this->upload->data()); 
			$post_arr["file"] = 'news_file/'.$post_arr['type']."/".$post_arr['lang']."/".$data["upload_data"]["file_name"];	
		} else{ 
			$post_arr["file"] = '';				 
		} 
	...
	{ 
		...
		if ($this->upload->do_upload('file'))
		{
			...
			$post_arr["file"] = $post_arr["exist_file"];					 
		} 
